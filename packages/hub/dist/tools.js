...
export function createToolset(store) {
    return [
        {
            name: "register_agent",
            description: "Announce yourself to the hub. Call this FIRST, immediately after starting. Idempotent.",
            shape: {
        ...
        {
            name: "post_plan",
            description: "PLANNING phase: propose your work breakdown as structured items (≤15). Include EVERYTHING the goal needs — code, env vars, migrations, deployment. Leave unclear items unassigned (no ownerHint). Re-posting replaces your previous proposal.",
            shape: {
                agent_id: z.string(),
                items: z.array(planItemSchema).describe("Task items. Titles + paths only — no essays."),
            },
            handler: (a) => store.postPlan(String(a.agent_id), (a.items ?? [])),
        },
        {
            name: "await_plan_approval",
            description: "PLANNING phase: block until the human approves the merged plan. If it returns {pending:true, retry:true}, call it again — do not give up, do not start coding.",
            shape: { agent_id: z.string() },
        ...
        {
            name: "get_board",
            description: "Read the task board. Pass since_version (from your last read) to receive only changed items — never re-read the whole board.",
        ...
        {
            name: "complete_task",
            description: "EXECUTING phase: mark a claimed item done. Pass refs — file paths the other agent should look at.",
            shape: {
                agent_id: z.string(),
                task_id: z.string(),
                refs: z.array(z.string()).optional().describe("File paths, workspace-relative. Not code."),
            },
            handler: (a) => store.completeTask(String(a.agent_id), String(a.task_id), a.refs),
        },
        {
            name: "post_contract",
            description: "Share an interface/agreement (API shapes, schemas, formats) the moment it is DECIDED — before it is implemented. Versioned, hashed, mirrored to contracts/<service>.md in your workspace.",
            shape: {
                agent_id: z.string(),
            ...
            handler: (a) => store.postContract(String(a.agent_id), String(a.content ?? ""), a.title === undefined ? undefined : String(a.title), a.service === undefined ? undefined : String(a.service)),
        },
        {
            name: "get_contracts",
            description: "Read contracts. Pass since_version (from your last read) for deltas. Call when you are ready to integrate — not in a polling loop.",
            shape: { since_version: z.number().optional() },
        ...
        },
        {
            ...
            handler: (a) => store.postUpdate(String(a.agent_id), String(a.message ?? ""), a.refs),
        },
        {
            name: "get_logs",
        ...
        },
        {
            ...
            description: "CHECKPOINT mode: pause before/after major or irreversible work. Two lines: what you did, what you'll do next. Then call get_checkpoint_status.",
            shape: {
                agent_id: z.string(),
        ...
        },
        {
            ...
            description: "Check your pending checkpoint: pending | approved | feedback (+ feedback text). Pass wait:true to block until resolved (retry on {pending:true}).",
            shape: { agent_id: z.string(), wait: z.boolean().optional() },
            handler: (a) => a.wait === true
                ? store.awaitCheckpoint(String(a.agent_id), longPollMs())
                : store.getCheckpointStatus(String(a.agent_id)),
        },
        {
            name: "get_resume_brief",
            description: "Condensed restart context: goal, your remaining items, contracts in force, peer completions, latest feedback. Call this first when resuming after an interruption.",
            shape: { agent_id: z.string() },
...
export function toTextResult(value) {
