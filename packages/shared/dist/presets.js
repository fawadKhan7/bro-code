...
export const PRESETS = {
    "frontend-backend": {
        ...
        roleA: "Frontend",
        roleB: "Backend",
        focusA: "UI, client flows, forms, routing, and browser token storage. Use get_contracts before integrating APIs. Post UI contracts and component paths via post_update with refs.",
        focusB: "REST/GraphQL routes, auth, persistence, and server validation. Publish endpoints and schemas early with post_contract so your peer can integrate.",
    },
    "builder-reviewer": {
        id: "builder-reviewer",
        ...
        focusB: "Review Builder's updates via get_contracts. Focus on correctness, security, and tests. Post feedback-style updates; avoid large rewrites without checkpoint.",
    },
 ...
 */
export function buildBriefs(goal, preset, overrides) {
    if (overrides?.briefA && overrides?.briefB) {
        return { briefA: overrides.briefA, briefB: overrides.briefB };
    }
    const bias = "This is a starting bias, not a boundary — if the plan surfaces work outside it that lives in your workspace, claim it.";
    if (preset === "custom") {
