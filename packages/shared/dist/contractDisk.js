...
export function sanitizeServiceSlug(raw) {
    let s = raw
        .toLowerCase()
        .replace(/[^a-z0-9-]+/g, "-")
        .replace(/^-+|-+$/g, "")
...
export function parseServiceFromContent(content) {
    const m = content.match(/^\s*service:\s*([^\s\r\n]+)/im);
    if (!m)
        return undefined;
    return sanitizeServiceSlug(m[1]);
}
export function resolveServiceSlug(content, explicit) {
    if (explicit?.trim())
...
export function writeContractRevisionToDisk(workspaceRoot, slug, entry) {
    const dir = path.join(workspaceRoot, "contracts");
    fs.mkdirSync(dir, { recursive: true });
    const filePath = path.join(dir, `${slug}.md`);
    const titleLine = entry.title ? `**Title:** ${entry.title}\n` : "";
    const block = `<!-- duo contract revision — safe to commit after redacting secrets -->\n` +
