/** Resume brief — condensed restart context, a computed view over session state.
 *  Refs only, never code. See docs/03-core-concepts/resume-system.md.
 */
import type { SessionState } from "./types.js";

export function buildResumeBrief(state: SessionState, agentId: string): string {
  ...
  const mine = state.board.filter(
    (i) => (i.owner === agentId || i.claimedBy === agentId) && i.status !== "done" && i.status !== "out-of-scope"
  );
  ...
  if (state.contracts.length === 0) {
    ...
    const latest = new Map<string, (typeof state.contracts)[number]>();
    for (const c of state.contracts) latest.set(c.service, c);
    for (const c of latest.values()) {
      lines.push(
        `  ${c.service} rev${c.revision} (sha256:${c.contentHash.slice(0, 8)}…)${c.diskPath ? ` → ${c.diskPath}` : ""}`
      );
    }
  }
  lines.push("");
