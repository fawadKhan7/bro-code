/** Project scanner — ported from Conductor (conductor/shared/src/scan.ts).
 *  Regex extraction over file text (not a real AST). Zero LLM, zero tokens to produce.
 *  Known v1 limits (backlog): JS/TS only; Express-style routes only.
 */
...
function scanFile(rel: string, full: string): string[] {
  ...
  if (routes.length && exports.length) {
    hints.push(`${relPosix}  ← exports: ${exports.join(", ")}; routes: ${routes.join(", ")}`);
  } else if (routes.length) {
    hints.push(`${relPosix}  ← routes: ${routes.join(", ")}`);
...
function walk(root: string, rel: string, depth: number, acc: string[]): void {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      acc.push(...scanFile(r, path.join(root, r)));
    }
  }
}

/** Plain-text project map for kickoff injection. Empty string if nothing useful. */
