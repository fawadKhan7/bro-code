...
async function appendGitignorePatterns(workspaceUri, patterns) {
...
async function installProtocolRules() {
    ...
    const body = (0, mcpToolCatalog_1.buildDuoProtocolMdc)();
    const target = vscode.Uri.joinPath(rulesDir, "duo-protocol.mdc");
    await vscode.workspace.fs.writeFile(target, Buffer.from(body, "utf8"));
    const choice = await vscode.window.showInformationMessage("Duo Agent: wrote .cursor/rules/duo-protocol.mdc (generated from the live MCP tool catalog). Re-run this command after upgrading the extension.", "Add contracts/ & duo-digest.md to .gitignore", "Close");
...
function apiGet(apiPath) {
    return new Promise((resolve, reject) => {
        const req = http.get({ hostname: "127.0.0.1", port: MCP_PORT, path: apiPath }, (res) => {
            let raw = "";
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("duo-agent.panel", provider, {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("duo-agent.startServer", () => provider.startServer()), vscode.commands.registerCommand("duo-agent.stopServer", () => provider.stopServer()), vscode.commands.registerCommand("duo-agent.openPanel", () => {
        vscode.commands.executeCommand("duo-agent.panel.focus");
    }), vscode.commands.registerCommand("duo-agent.openControlTerminal", () => provider.openControlTerminal()), vscode.commands.registerCommand("duo-agent.installProtocolRules", installProtocolRules));
...
// ── View provider ───────────────────────────────────────────────────────────
class DuoAgentViewProvider {
    constructor(_extensionUri) {
        this._extensionUri = _extensionUri;
        this._updateReq = null;
    }
    ...
    openControlTerminal() {
        if (proxyMode) {
            vscode.window.showWarningMessage("Duo Agent: Open Session Terminal in the Cursor window where you started the server (this window only has the client).");
            return;
        }
        ...
        if (!fs.existsSync(cliPath)) {
            vscode.window.showErrorMessage(`Duo Agent: Session CLI missing. Run npm run compile. Expected: ${cliPath}`);
            return;
        }
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case "disconnectAgent":
                    await this.disconnectAgent(msg.agentId);
                    break;
                case "openControlTerminal":
                    this.openControlTerminal();
                    break;
                case "resolveCheckpoint":
                    await this.resolveCheckpoint(msg.agentId, msg.approved, msg.feedback);
                    break;
                case "showError":
                    vscode.window.showErrorMessage(msg.text);
                    break;
                case "submitTask":
                    await this.submitTaskFromPanel(msg.agentId, String(msg.instructions ?? ""), msg.paths ?? [], msg.mode === "checkpoint" ? "checkpoint" : "auto-run");
                    break;
                case "pickScopeFiles":
                    await this.pickScopeFiles(msg.agentId);
                    break;
                case "addOpenEditorsToScope":
                    this.addOpenEditorsToScope(msg.agentId);
                    break;
            }
        });
    }
    async submitTaskFromPanel(agentId, instructions, paths, mode) {
        ...
        try {
            if (proxyMode) {
                const { status, data } = await apiPost("/api/task", {
                    agent_id: agentId,
                    task: full,
                    mode,
                });
                ...
                const { data: st } = await apiGet("/api/status");
                const ws = agentId === "A" ? st.workspaceA : st.workspaceB;
                if (ws)
                    (0, taskFileWriter_1.writeTaskFilesToDisk)(ws, agentId, full, mode);
            }
            else {
                if (!mcpServer?.isRunning()) {
            ...
            vscode.window.showInformationMessage(`Duo Agent: task saved for Agent ${agentId}. In each window, Agent chat → get_my_task (poll if waiting).`);
    ...
    async startServer(silent = false) {
        if (mcpServer?.isRunning() || proxyMode) {
            if (!silent)
                vscode.window.showInformationMessage("Duo Agent: MCP server is already running.");
            return;
        }
        ...
        if (alreadyUp) {
            ...
            this.subscribeToUpdates();
            if (!silent)
                vscode.window.showInformationMessage("Duo Agent: Connected to existing MCP server.");
            return;
        }
        mcpServer = new mcpServer_1.DuoAgentMcpServer((event) => {
    ...
    async stopServer() {
        if (proxyMode) {
            vscode.window.showWarningMessage("Duo Agent: Stop the server from the Cursor window that started it (this window is a client only).");
            return;
        }
        if (!mcpServer?.isRunning())
            return;
    ...
    subscribeToUpdates() {
        ...
        this._updateReq = http.get(`http://127.0.0.1:${MCP_PORT}/api/updates`, (res) => {
            let buffer = "";
            res.on("data", (chunk) => {
    ...
    async connectAgent() {
        ...
        if (proxyMode) {
            try {
                const { data, status } = await apiPost("/api/register", { workspace_path: workspacePath });
    ...
    // ── Checkpoint ─────────────────────────────────────────────────────────────
    async resolveCheckpoint(agentId, approved, feedback) {
        if (proxyMode) {
