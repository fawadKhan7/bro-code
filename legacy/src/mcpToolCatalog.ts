...
export const DUO_MCP_TOOL_SCHEMAS = [
  {
    name: "register_agent",
    description:
      "Register this Cursor window as an agent in the Duo Agent session. Returns the assigned agent ID (A or B) and workspace path. Call this once at the start of a session.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_path: {
  ...
  },
  {
    name: "get_my_task",
    description:
      "Returns the task for this agent (A or B). First successful call from each agent window records that agent as ready. Until BOTH agents have called get_my_task once, the response includes waiting: true — then poll again to receive the full task.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        message: {
          type: "string",
          description:
            "Progress message. If blocked waiting on the other agent, start with BLOCKED: and explain.",
        },
      },
  ...
  },
  {
    name: "post_contract",
    description:
      "Share an interface, data format, or agreement with the other agent. Also writes contracts/<service>.md under this agent's workspace (immediate disk mirror).",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string", description: "Agent ID — 'A' or 'B'" },
        content: {
          type: "string",
          description:
            "Contract body. Optionally start with a line service: <slug> (slug: lowercase letters, digits, hyphens).",
        },
        title: { type: "string", description: "Short human-readable title for this contract revision." },
        service: {
          type: "string",
          description:
            "Service slug for filename (e.g. auth-api). If omitted, parsed from content line service: <slug> or a stable default slug is used.",
        },
      },
  ...
  },
  {
    name: "get_contracts",
    description:
      "Returns all shared contracts posted by both agents in this session, each with contentHash, timestamp, optional title/service, and revision version for staleness checks.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "post_checkpoint",
    description:
      "Pause at a milestone and request user approval before continuing (Checkpoint mode only).",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string", description: "Agent ID — 'A' or 'B'" },
...
function schemaSummary(schema: { properties?: Record<string, { description?: string }>; required?: string[] }): string {
...
export function buildDuoProtocolMdc(): string {
  const toolBlocks = DUO_MCP_TOOL_SCHEMAS.map((t) => {
    const props = t.inputSchema as { properties?: Record<string, { description?: string }>; required?: string[] };
