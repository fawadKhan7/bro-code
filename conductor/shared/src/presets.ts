...
export const PRESETS: Record<Exclude<PresetId, "custom">, RolePreset> = {
  "frontend-backend": {
    ...
    roleB: "Backend",
    focusA:
      "UI, client flows, forms, routing, and browser token storage. Use get_contract before integrating APIs. Post UI contracts and component paths via post_update with refs.",
    focusB:
      "REST/GraphQL routes, auth, persistence, and server validation. Publish endpoints and schemas early with post_update + refs so Agent A can integrate.",
  },
  "builder-reviewer": {
    id: "builder-reviewer",
    ...
    focusB:
      "Review Builder's updates via get_contract. Focus on correctness, security, and tests. Post feedback-style updates; avoid large rewrites without checkpoint.",
  },
...
export function buildBriefs(
  goal: string,
  preset: PresetId,
  overrides?: { briefA?: string; briefB?: string; customRoleA?: string; customRoleB?: string }
